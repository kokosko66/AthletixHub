...
export const getMealPlanById = async (req, res) => {
    try {
        ...
        if (rows.length === 0) return res.status(404).json({ message: 'Meal Plan not found' });
        res.json(rows[0]);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
export const addMealPlan = async (req, res) => {
    try {
        ...
        await pool.query(queries.addMealPlan, [name, created_at]);
...
export const updateMealPlan = async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) return res.status(404).json({ message: 'Meal Plan not found' });
